...
#           add logic to check the save or the restart and code the restarting routine
            ...
            return render_template(f"{TEMPLATE_PATH}/config.html", domain=AP_DOMAIN, ap_wifi=ap_wifi, ssid=ssid,
                                   password=password, front_camera_ip=front_camera_ip, back_camera_ip=back_camera_ip,wifi_networks_options=wifi_options)
...
def application_mode():
    ...
    def app_app(request):
#          it has to read from the file the real ip addresses
        with open(CONFIG_FILE) as f:
            file = json.load(f)
            front_camera_ip = file["front_camera_ip"]
            back_camera_ip = file["back_camera_ip"]
...
#           add logic to check the save or the restart and code the restarting routine
            if 'save_restart' in request_string:
                ...
                return render_template(f"{TEMPLATE_PATH}/config.html", ip=IP_ADDRESS,saved=1, domain=AP_DOMAIN, ap_wifi=ap_wifi,
                                       ssid=ssid, password=password, front_camera_ip=front_camera_ip,
                                       back_camera_ip=back_camera_ip,wifi_networks_options=wifi_options,speed=speed)
            else:
                #restart routine
            ...
            return render_template(f"{TEMPLATE_PATH}/config.html", ip=IP_ADDRESS,domain=AP_DOMAIN, ap_wifi=ap_wifi, ssid=ssid,speed=speed,
                                   password=password, front_camera_ip=front_camera_ip, back_camera_ip=back_camera_ip,wifi_networks_options=wifi_options)
...
#                print(request.data.get(key))
            ...
            if joypad == 'top': # camera servos
                ...
                if scaledX > 0 and scaledY > 0:
                    motor_car.move_forward_api()
                    motor_car.move_right_api()
                    print("scaledX > 0 and scaledY > 0")
                     
                elif scaledX < 0 and scaledY > 0:
                    motor_car.move_forward_api()
                    motor_car.move_left_api()
                    print("scaledX < 0 and scaledY > 0")
                
                elif scaledX < 0 and scaledY < 0:
                    motor_car.move_backward_api()
                    motor_car.move_left_api()
                    print("scaledX < 0 and scaledY < 0")
    ...
    server.add_route("/api/distance", handler=app_api_distance, methods=["GET"])
